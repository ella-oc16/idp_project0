...
unsigned long current_millis, previous_millis; //time measurement for LED blinks
const unsigned long interval = 500;   //interval between LED blinks
...
void loop() {
    ...
    if (current_millis - previous_millis >= interval){
        ...
        if (led_orange_state == LOW) {
            led_orange_state = HIGH;
        } else {
            led_orange_state = LOW;
        }
        previous_millis = current_millis;
        digitalWrite(led_orange, led_orange_state);
    }
    ...
    switch(position){
        ...
        case 3:    //while in the tunnel, check still in tunnel and drive forward
            while (is_tunnel == 1){
                ...
                middle_sensor_state = digitalRead(middle_sensor_pin);
                right_sensor_state = digitalRead(right_sensor_pin);
                navigation(left_sensor_state, middle_sensor_state, right_sensor_state);  //when no line, navigation drives forward
                is_tunnel = tunnel_detection();
            }
            position  = 4;
            break;

        case 4: //after the tunnel, check for block, depending on block number
            if (block_no == 0){
                //Send a pulse out of the trigger
                digitalWrite(ultrasonic_trigger_pin, HIGH);
                delay(10);
